...
import static yxs.Sorts.SortUtils.print;
...
class CountingSort implements SortAlgorithm {
	 ...
	 *            <p>
	 *            Sorts the list in increasing order The method uses list elements
	 *            as keys in the frequency map
	 **/
	@Override
	...
	/**
	 * Stream Counting Sort The same as method {@link CountingSort#sort(List)} } but
	 * this method uses stream API
	 *
	 * @param list
	 *            The list to be sorted
	 **/
	private static <T extends Comparable<T>> List<T> streamSort(List<T> list) {
		return list.stream().collect(toMap(k -> k, v -> 1, (v1, v2) -> v1 + v2, TreeMap::new)).entrySet().stream()
